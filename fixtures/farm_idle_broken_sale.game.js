// DELIBERATELY BROKEN VARIANT for tests/test_farm_idle_adversarial.py
// Broken: _sellAtStall() no longer adds CROP_SELL_VALUE to this.coins
// (selling clears carrying but never pays out)
...
const PLOT_UPGRADE_COST_GROWTH = 1.5;
...
class PlayScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Initialize plots
    const startX = (W - (PLOT_COUNT * PLOT_SIZE + (PLOT_COUNT - 1) * PLOT_GAP)) / 2;
    for (let i = 0; i < PLOT_COUNT; i++) {
      const x = startX + i * (PLOT_SIZE + PLOT_GAP);
      const y = PLOT_AREA_TOP + 60;
      this._createPlot(i, x, y);
    }
  ...
  _sellAtStall() {
    ...
    // BROKEN: does NOT add CROP_SELL_VALUE to this.coins
    // this.coins += CROP_SELL_VALUE;  // <-- THIS LINE REMOVED ON PURPOSE
    ...
    Juice.Particles.burst(this, STALL_X + STALL_W / 2, STALL_Y, {
      color: 0xffd700, count: 12, speed: 120, lifespan: 500, size: 6
    });
  ...
  _buyUpgrade(type) {
    ...
    if (type === 'plot') {
      cost = Math.ceil(PLOT_UPGRADE_BASE_COST * Math.pow(PLOT_UPGRADE_COST_GROWTH, this.plotUpgradeCount));
    ...
    if (type === 'plot') {
      ...
      if (row) {
        const newCost = Math.ceil(PLOT_UPGRADE_BASE_COST * Math.pow(PLOT_UPGRADE_COST_GROWTH, this.plotUpgradeCount));
    ...
    Juice.Particles.burst(this, this.farmer.x, this.farmer.y, {
      color: type === 'plot' ? 0x8bc34a : type === 'boots' ? 0xffb300 : 0xff7043,
      count: 10, speed: 100, lifespan: 400, size: 5
    });
  }

  _addNewPlot() {
    const lastPlot = this.plots[this.plots.length - 1];
    const newX = lastPlot.x + PLOT_SIZE + PLOT_GAP;
    const newY = PLOT_AREA_TOP + 60;
    this._createPlot(this.plots.length, newX, newY);
  }
  ...
  _getCheapestAffordableUpgrade() {
    const upgrades = [];
    // Plot upgrade
    const plotCost = Math.ceil(PLOT_UPGRADE_BASE_COST * Math.pow(PLOT_UPGRADE_COST_GROWTH, this.plotUpgradeCount));
  ...
  _updateHelper(helper, dt) {
    ...
    if (helper.state === 'moving_to_stall') {
      ...
      if (dist < 30) {
        ...
        Juice.Particles.burst(this, STALL_X + STALL_W / 2, STALL_Y, {
          color: 0xffd700, count: 8, speed: 80, lifespan: 400, size: 5
        });
        // CTA on first sale
        if (CTA_ENABLED && !this.ctaButton) {
          this._createCTAButton();
