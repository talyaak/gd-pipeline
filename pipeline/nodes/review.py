...
IMPORTANT — the code below is the codegen output BEFORE the execution harness injects vendor libraries. These globals are ALWAYS injected server-side before the game runs and are guaranteed to exist at runtime even though their source isn't visible in the code you're reviewing: `window.Phaser`, `window.ParticleEngine` (and `Vector2`/`Particle`/`ParticleEmitter`), `window.Art` (and `SoftCircle`/`SoftRect`/`Noise`/`Gradient`), `window.UIKit` (and its `Button`/`ProgressBar`/`Popup` classes), `window.Juice` (and its `ScreenEffects`/`ParticleBurst`/`TrailEffect`/`ParallaxBackground`/`ScoreDisplay`/`EndCard`/`ScorePopup`/`LaneGuides`/`MraidGate`/`AudioHelper` classes), and `window.mraid` when present. Do NOT flag references to these as "undefined", "never defined or imported", or "missing dependency" — that's a false positive caused by not seeing the injection step, not a real bug. Only flag a genuine usage bug if the code misuses one of these APIs (wrong constructor signature, wrong method name, etc.), not merely for referencing it.
