...
- A juice/toolkit library is available as `window.Juice` with reusable game-juice utilities: `ScreenEffects` (shake, flash, hitPause), `ParticleBurst` (emitBurst, pickupSparkle, matchExplosion), `TrailEffect`, `ParallaxBackground`, `ScoreDisplay`, `EndCard`, `ScorePopup`, `LaneGuides`, `MraidGate`, `AudioHelper`. Use these instead of implementing your own juice effects.
