  ...
  _updateHelper(helper, dt) {
    ...
    if (helper.state === 'moving_to_plot') {
      ...
      if (dist < MAGNET_RADIUS) {
        ...
        // Same carry-stack cap as the player's own harvest (defect #1) --
        // the helper adds to the shared player carry stack (_addToCarryStack
        // below), so it must respect the same bound.
        const carryFull = this.carrySprites.length >= CARRY_STACK_MAX;
        if (plot && !plot.harvested && this.time.now >= plot.readyAt && !carryFull) {
          plot.harvested = true;
          plot.cropSprite.setVisible(false);
          if (plot.pulseTween) {
            plot.pulseTween.stop();
        ...
        } else if (carryFull && this.carrySprites.length > 0) {
          // Carry stack is full -- go deposit at the market instead of
          // idling right next to a plot it can't harvest.
          helper.state = 'moving_to_stall';
          const stall = this._computeStallLayout();
          helper.target = { x: stall.centerX, y: stall.centerY };
        } else {
