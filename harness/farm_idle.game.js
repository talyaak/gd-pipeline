...
class PlayScene extends Phaser.Scene {
  ...
  _flyCoinsToCounter(amount, sourceX, sourceY) {
    ...
    // farmer.x/y). The coin sprite itself is a HUD object (its whole flight
    // ends at coinsText, a HUD object, and it must visually track over
    // in-world content correctly regardless of the world camera's current
    // scroll) -- so its start position is converted from world-space to
    // UI-camera-space by subtracting the world camera's current scroll.
    // Without this, the coin would start at the wrong on-screen position
    // the moment the world camera has scrolled away from (0,0), even
    // though it visually needs to start exactly where the source
    // (helper/farmer) currently appears on screen.
    const worldCam = this.cameras.main;
    const startX = sourceX - worldCam.scrollX;
    const startY = sourceY - worldCam.scrollY;
